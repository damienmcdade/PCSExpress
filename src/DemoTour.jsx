...
const DEMO_STEPS = [
  ...
  },
  {
    ...
    content: 'Inside Home Relocation → Shipment Tracker:\n• Enter your GBL / TCN, TSP name, contact phone, and your spread windows.\n• Hit "Start tracking" and tap "Mark done" as each milestone clears: counseling → TSP assigned → pre-move survey → packing → loaded → in transit → arrival call → delivered → claim filed → claim settled.\n• Every stage has a published JTR service-level window — if a stage runs past the window, the app flags it amber and tells Marcus to escalate to the gaining PPSO or the TSP.\n• Turn on reminders to get a local alert for overdue stages the next time you open the app.\n• All shipment metadata is encrypted (AES-256) on the device. Nothing is sent to a PCS Express server. Deep links route directly to dps.move.mil and the customer portal so the next official action is always one tap away.',
    action: 'Go to Home Relocation',
    target: 'home-relocation',
  },
  {
    id: 'inventory-vault',
    title: 'Inventory & Claims Worksheet',
    description: 'A text-only HHG inventory you can hand to a claim adjuster. PCS Express does not accept or store photos — keep any supporting pictures in your own camera roll.',
    content: 'Inside Home Relocation → Inventory & Claims:\n• Walk every room before pack-out and record each item: name, room, declared replacement value, condition, free-text notes.\n• Switch to the "post-delivery" phase at arrival and re-walk the same rooms. Anything you flag Damaged or Poor is highlighted in the totals.\n• Tap "Export DD 1840R-ready claim worksheet" — the app builds a printable PDF with every room, every item, totals, and a damage summary. Browser Print → Save as PDF, then attach to the DPS claim.\n• Filing window: report loss/damage within 180 days, then file the claim within 9 months of delivery for full replacement value.\n• The inventory text is AES-256 encrypted on the device and never uploaded. Photos are not part of the workflow — store your own supporting photos in your phone\'s camera roll outside the app, and reference them in the Notes field by description.',
    action: 'Go to Home Relocation',
    target: 'home-relocation',
  },
  {
