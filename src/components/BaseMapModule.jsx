  ...
  useEffect(() => {
    ...
    if (facilities.length === 0) {
      L.popup()
        ...
        .setContent(`<b>${baseInfo.name}</b><br/>${baseInfo.branch} · ${baseInfo.state}<br/><small>Public support map only. For official installation directories, visit:<br/><a href="https://www.militaryinstallations.dod.mil/" target="_blank">militaryinstallations.dod.mil</a></small>`)
        .openOn(map);
    }
    ...
    <div style={{ fontFamily: 'system-ui', background: '#f0f4f8', minHeight: '100%' }}>
      ...
      <div style={{ padding: '12px 14px', background: theme.secondary, borderBottom: `1px solid ${theme.accent}30` }}>
        ...
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${theme.accent}25` }}>
          ...
          {manualBase && (
            <a href={`https://www.google.com/search?q=${encodeURIComponent(`${manualBase} official public installation directory services`)}`} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 8, fontSize: 10, color: theme.accent, fontWeight: 800, textDecoration: 'none' }}>
              Search Google public base information →
            </a>
          )}
        </div>
      </div>
      ...
      {noData && (
        <div style={{ padding: 24, textAlign: 'center', color: '#888', fontSize: 12 }}>
          ...
          <div style={{ fontSize: 11, marginTop: 4 }}>Enter a base above to geocode a public map center and open Google public-source lookup links.</div>
        </div>
      )}
    </div>
  );
