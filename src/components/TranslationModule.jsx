...
// Shown when the user declines to send their text to the AI providers. Free
// text translation is the only thing that stops working — Common Phrases runs
// on-device — so the message says how to turn it back on rather than reading
// as an error.
const CONSENT_DECLINED_MESSAGE = 'Free-text translation is switched off because this text has not been cleared to go to an AI provider. The Common Phrases tab still works — those translations run on this device. To turn free-text translation on, tap Translate again and choose “Agree and continue”, or open Security & data handling at the bottom of the Command Center.'
...
// Sentinel return value: the user was asked for AI consent and declined. The
// caller shows a "translation is switched off" message instead of an error,
// because nothing failed — the user chose not to send the text.
const CONSENT_DECLINED = '__pcs_ai_consent_declined__';

async function callAI(system, user, onDelta, signal, allowConsentRetry = true) {
  try {
    ...
    // 451 — the server refused to transmit because no AI consent is on record
    // (Apple 5.1.2(i); see shared/aiConsent.js). Raise the sheet, then retry
    // this exact call once. Backstop for a stale tab whose local consent state
    // no longer matches what the server will accept.
    if (res.status === 451 && allowConsentRetry) {
      const granted = await requestAiConsent();
      if (!granted) return CONSENT_DECLINED;
      return callAI(system, user, onDelta, signal, false);
    }
...
export default function TranslationModule({ theme, profile }) {
  ...
  const handleTranslate = useCallback(async () => {
    ...
    // Apple 5.1.2(i): explicit consent BEFORE the first transmission, not
    // after the server bounces it. The 451 handling inside callAI stays as the
    // backstop for a device whose recorded consent no longer satisfies the
    // server.
    if (!hasAiConsent()) {
      const granted = await requestAiConsent();
      if (!granted) {
        setResult(CONSENT_DECLINED_MESSAGE);
        return;
      }
    }
    setLoading(true);
        ...
        <div>
          ...
          {result && (
            <div style={{ background: '#FFF', border: '1px solid #E0E6EE', borderLeft: `3px solid ${theme.primary}`, borderRadius: 12, padding: 14, marginBottom: 12 }}>
              ...
              {/* Permanent AI-content disclosure. Names the same providers the
                  consent sheet names, because the failover to OpenAI is real. */}
              <div style={{ marginTop: 10, fontSize: 11, color: '#46586B', lineHeight: 1.5, borderTop: '1px solid #EEF2F6', paddingTop: 8 }}>
                AI-generated by Anthropic (Claude), or by OpenAI when Anthropic is unavailable. Machine translation can be wrong — check anything official against a qualified interpreter or the office that issued the document.
              </div>
            </div>
          )}
