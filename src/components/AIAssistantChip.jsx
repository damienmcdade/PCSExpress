...
// curated knowledge base and every non-AI part of the app keep working, so
// this reads as a setting, not a failure.
const AI_CONSENT_DECLINED_MESSAGE = 'The AI Assistant is switched off because your question has not been cleared to go to an AI provider. Everything else in PCS Express still works, and the JTR Assistant tab inside Movement & Logistics has a curated JTR / FTR / DSSR library with citations. To turn the assistant on, send your question again and choose “Agree and continue”, or open Security & data handling at the bottom of the Command Center.';
...
export function AIAssistantModal({ open, onClose, isDesktop, language = 'en', userContext = null }) {
  ...
  const submit = async () => {
    ...
    if (!q || busy) return;
    // Apple 5.1.2(i): explicit consent before this question, the conversation
    // history, and the move-context blob are transmitted to a third-party AI.
    // The question stays in the box on decline so a change of mind is one tap.
    if (!hasAiConsent()) {
      const granted = await requestAiConsent();
      if (!granted) {
        setMessages(prev => [...prev, { role: 'system', text: AI_CONSENT_DECLINED_MESSAGE }]);
        return;
      }
    }
    setQuestion('');
    ...
    try {
      ...
      let timer = setTimeout(() => abortRef.current?.abort(), 30_000);
      ...
      const send = () => fetch(apiUrl('/api/jtr-assistant'), {
        method: 'POST',
      ...
      let r = await send();
      // 451 — the server refused to transmit because no AI consent is on
      // record (shared/aiConsent.js). Raise the sheet, then replay this exact
      // question once. The in-flight timer is cleared and restarted around the
      // prompt so the 30s budget isn't spent while the user reads the sheet.
      if (r.status === 451) {
        clearTimeout(timer);
        const granted = await requestAiConsent();
        if (!granted) {
        ...
        abortRef.current = new AbortController();
        timer = setTimeout(() => abortRef.current?.abort(), 30_000);
        r = await send();
      }
    ...
    >
      ...
      >
        ...
        <div style={{ background: '#EEF2F6', color: '#46586B', padding: '6px 14px', fontSize: 10.5, lineHeight: 1.45, borderBottom: '1px solid #DDE3EA' }}>
          Answers are AI-generated by Anthropic (Claude), or by OpenAI when Anthropic is unavailable. They are a planning aid and can be wrong or out of date — verify any dollar amounts, dates, and eligibility against the official DTMO / GSA / IRS source before acting. Not financial, tax, or legal advice.
        </div>
